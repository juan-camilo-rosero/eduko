  ...
  return (
    <div className={`fixed h-screen bg-black/60 backdrop-blur-2xl w-full left-0 border-t-4 border-blue-turquoise z-40 flex flex-col items-center justify-between pt-28 px-6 gap-8 pb-12 transition-all md:px-16 lg:px-24  ${(account)
        ...
        <div className="w-full flex flex-col gap-8 mb-8 items-center lg:gap-4 lg:mb-0">
            <button className="w-full py-2 neon-white-box transition-all hover:bg-white hover:text-black text-light text-2xl rounded-xl font-semibold md:w-1/2 lg:w-1/4" onClick={() => handleUpdate()}>update data</button>
            <button className="w-full py-2 neon-white-box transition-all hover:bg-white hover:text-black text-light text-2xl rounded-xl font-semibold md:w-1/2 lg:w-1/4" onClick={() => handleLogout()}>logout</button>
            <button className="w-full py-2 neon-turquoise-box transition-all hover:bg-blue-turquoiseHover text-blue-turquoise hover:text-black text-2xl rounded-xl font-semibold md:w-1/2 lg:w-1/4" onClick={() => setAccount(false)}>cancel</button>
        </div>
    </div>
  )
}
